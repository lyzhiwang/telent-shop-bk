  ...
  >
    <el-form ref="tagForm" :rules="rulesForm" :model="tagForm" label-width="100px">
      ...
      <el-form-item label="标签类型" prop="type">
        <el-select v-model="tagForm.type" placeholder="请选择标签类型" clearable>
          <el-option v-for="item in typeList" :key="item.id+item.name" :label="item.name" :value="item.id" />
        </el-select>
      </el-form-item>
      <el-form-item>
  ...
  methods: {
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.apiBtn(this.type === 1 ? 'TagStore' : 'TagUpdate', { ...this.tagForm })
            .then(res => {
              this.$emit('change')
