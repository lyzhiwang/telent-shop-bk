...
export default {
  ...
  mounted () {
    this.mediaForm.activity_id = this.$route.query.id
  },
  methods: {
    close () {
      this.fileList = []
      this.mediaForm.medias = []
    ...
    changeVideo (arr) {
      this.mediaForm.medias = []
      arr.map(v => {
          let obj = { upload_id: v.id, type_id: this.typeId }
          this.mediaForm.medias.push(obj)
      })
    },
    submit () {
      if (this.mediaForm.medias.length > 0) {
        this.apiBtn('ActivityStoreMedia', this.mediaForm).then(res => {
